...
void main() {
	...
	// float currentDepth = currentClipPos.w;
    // float previousDepth = previousClipPos.w;
	// float velocityZ = currentDepth - previousDepth; // blocks per frame. < 0 when approaching

	// motion = vec4(pixelVelocity, velocityZ, 1.0);
}